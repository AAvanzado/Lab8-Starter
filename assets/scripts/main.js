...
function initializeServiceWorker() {
  ...
  if ('serviceWorker' in navigator){
    ...
    try{
      ...
      console.log ('service worker rgistration failed :( ');
    }
    });
  }
  else{
...
async function getRecipes() {
  ...
  return new Promise((resolve,reject) => {
    ...
    //            declared above
    RECIPE_URLS.forEach(async (link) =>{
      ...
      try{
        ...
        //            resolve() method.
        if(reqRecipes.length === RECIPE_URLS.length){
          saveRecipesToStorage(reqRecipes);
          resolve(reqRecipes);
        }
      }
      catch{
